...
import { ChevronDownIcon, Link, Bell, Shield, HelpCircle } from "lucide-solid";
...
export default function WithIconSubHeaderAndChevronAccordion() {
  ...
  return (
    <Accordion defaultValue={["mindful-living"]} collapsible class="w-full max-w-md mx-auto bg-gradient-to-br from-gray-100/80 to-gray-200/80 dark:from-gray-900/80 dark:to-gray-800/80 backdrop-blur-sm">
      <For each={items}>
        {(item) => (
          <Accordion.Item
            value={item.id}
          ...
          >
            <Accordion.ItemTrigger class="w-full px-4 py-3 flex items-center justify-between text-left hover:bg-gradient-to-r hover:from-blue-500/5 hover:to-purple-500/5 transition-all duration-200">
              <div class="flex items-center flex-1">
                <Dynamic component={item.icon} class="w-5 h-5 text-blue-600 dark:text-blue-400 mr-3 flex-shrink-0" />
                <div class="flex-1">
