...
export default function VerticalNavigationSteps() {
  const steps = [1, 2, 3, 4];
  ...
  return (
    <div class="bg-white dark:bg-gray-800 w-full px-4 py-12 rounded-xl flex items-center justify-center">
      ...
      >
        <Steps.List class="flex flex-col gap-4">
          <For each={steps}>
            {(step, index) => (
              <Steps.Item index={index()} class="relative flex items-start">
                ...
                <Steps.Separator
                  ...
                  class="absolute top-8 left-4 w-0.5 h-8 bg-gray-200 dark:bg-gray-700 data-complete:bg-blue-600"
                />
              </Steps.Item>
            )}
          </For>
        ...
        <div class="flex justify-center gap-3 mt-8">
          ...
          <Steps.NextTrigger class="px-4 py-2 text-sm font-medium text-gray-600 bg-white border border-gray-300 rounded-md hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed dark:text-gray-400 dark:bg-gray-800 dark:border-gray-600 dark:hover:bg-gray-700">
            Next step
          </Steps.NextTrigger>
        </div>
      </Steps.Root>
