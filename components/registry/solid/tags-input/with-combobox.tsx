    ...
    <div class="bg-white dark:bg-gray-800 w-full px-4 py-12 rounded-xl flex flex-col items-center">
      <TagsInput.RootProvider value={tagsInput} class="w-full max-w-md">
        <Combobox.RootProvider value={combobox}>
          <>
            <TagsInput.Label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              Frameworks
            </TagsInput.Label>
            <Combobox.Control class="relative">
              ...
              <Combobox.Trigger class="px-2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors absolute inset-y-0 right-0">
                <ChevronDown class="h-4 w-4" />
              </Combobox.Trigger>
            </Combobox.Control>
            {tagsInput().value.length > 0 && (
              <TagsInput.ClearTrigger class="mt-2 text-sm text-gray-500 hover:text-gray-700 transition-colors dark:text-gray-400 dark:hover:text-gray-200">
                Clear all
              ...
              <Combobox.Positioner>
                <Combobox.Content class="mt-1 max-h-60 w-full overflow-auto rounded-md bg-white dark:bg-gray-800 py-1 shadow-lg ring-1 ring-black ring-opacity-5 dark:ring-gray-600 focus:outline-hidden z-50">
                  ...
                  {collection().items.filter(
                    ...
                    <div class="py-2 pl-3 pr-9 text-sm text-gray-500 dark:text-gray-400">
