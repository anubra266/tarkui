...
<template>
  <div class="flex items-center justify-center min-h-32">
    <Listbox.Root
      ...
      :default-value="['dark-theme', 'notifications', 'two-factor']"
    ...
    >
      ...
      >
        <div
          v-for="[category, items] in Object.entries(groupedItems)"
          :key="category"
        >
          <div
          ...
          >
            <div class="flex items-center gap-3 flex-1">
              <component
                :is="setting.icon"
                :class="`w-5 h-5 ${getIconColor(setting.value)}`"
              />
          ...
          <div
            v-if="category !== 'Sync'"
            class="h-px bg-gray-200 dark:bg-gray-700 mx-2 my-2"
          />
        </div>
      </Listbox.Content>
    </Listbox.Root>
