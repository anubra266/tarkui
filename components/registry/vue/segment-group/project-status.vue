<template>
  ...
  >
    ...
    <div class="p-4">
      <SegmentGroup.Root
        :value="activeStatus"
        @value-change="handleStatusChange"
        orientation="horizontal"
        class="flex items-start flex-row gap-0 border-b border-gray-200 dark:border-gray-700"
      >
        <SegmentGroup.Indicator
          class="border-blue-500 bottom-0 border-b-2 transform translate-y-px w-[var(--width)] transition-all duration-200"
        />
        <SegmentGroup.Item
    ...
    <div class="p-4 border-t border-gray-200 dark:border-gray-700">
      <p class="text-xs text-gray-600 dark:text-gray-400">
        Showing:
        <span class="font-medium capitalize">{{ activeStatus }}</span> projects
      </p>
    </div>
  </div>
</template>
...
function handleStatusChange(details) {
  activeStatus.value = details.value || "active";
}
</script>
