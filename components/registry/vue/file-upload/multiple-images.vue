...
const handleFileChange = (e: { acceptedFiles: File[] }) => {
  acceptedFiles.value = e.acceptedFiles;
};
</script>
...
<template>
  ...
  >
    <FileUpload.Context v-slot="context">
      <div
        class="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl p-6 bg-gray-50 dark:bg-gray-800 min-h-64"
      >
        <!-- Header -->
        ...
        <FileUpload.Dropzone
          v-else
          class="flex flex-col items-center justify-center py-8 text-center cursor-pointer"
        >
          <div
