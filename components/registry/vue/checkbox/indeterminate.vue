...
const selectedFeatures = ref(["dark-mode"]);

// Calculate parent state based on selected features
const getParentState = computed(() => {
  if (selectedFeatures.value.length === 0) return false;
  if (selectedFeatures.value.length === features.length) return true;
...
function handleGroupChange(details) {
  selectedFeatures.value = details.values;
}
</script>
...
<template>
  <div class="space-y-4">
    <Checkbox.Root
      ...
      @checked-change="handleParentChange"
    ...
    <div class="ml-6">
      <Checkbox.Group
        ...
        @value-change="handleGroupChange"
        class="space-y-3"
      >
        <Checkbox.Root
