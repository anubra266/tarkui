<script lang="ts">
  ...
  import { Folder, FolderOpen, FileText, X } from "lucide-svelte";
...
<FileUpload.Root directory class="w-full max-w-2xl" maxFiles={Infinity}>
  <FileUpload.Context>
          ...
          <div class="space-y-3">
            ...
            <FileUpload.ItemGroup>
              <div class="space-y-2 max-h-64 overflow-y-auto">
                {#each context().acceptedFiles as file (file.name + file.webkitRelativePath)}
                  <FileUpload.Item {file}>
                    <div class="flex items-center gap-3 p-3 border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-900">
                      ...
                      <div class="w-8 h-8 flex items-center justify-center flex-shrink-0 rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
                        {#if file.type.startsWith("image/")}
                        ...
                        {:else}
                          ...
                          <IconComponent class="w-4 h-4 text-blue-500 dark:text-blue-400" />
                        {/if}
                      </div>
