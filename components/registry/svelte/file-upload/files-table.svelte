<script lang="ts">
  ...
  const defaultFiles = [
    ...
    new File(
      ["Welcome to Ark UI Svelte, this is a zip file`"],
      "showcase.zip",
      {
    ...
    new File(
      ["Welcome to Ark UI Svelte, this is an excel file`"],
      "data.xlsx",
      {
    ...
    new File(
      ["Welcome to Ark UI Svelte, this is an audio file`"],
      "audio.mp3",
      {
        type: "audio/mpeg",
      }
    ...
    new File(
      ["Welcome to Ark UI Svelte, this is a video file`"],
      "documentary.mp4",
      {
        type: "video/mp4",
      }
      ...
      <div class="space-y-4">
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-2">
            <FileUpload.Trigger class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 text-xs font-medium rounded-md hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-900 dark:focus:ring-gray-100 focus:ring-offset-2">
              <Upload class="w-3 h-3" />
              Add files
            </FileUpload.Trigger>
            {#if context().acceptedFiles.length > 0}
              <FileUpload.ClearTrigger class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 text-xs font-medium rounded-md hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-900 dark:focus:ring-gray-100 focus:ring-offset-2">
        ...
        {#if context().acceptedFiles.length > 0}
          <div class="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden bg-white dark:bg-gray-900">
            ...
            <FileUpload.ItemGroup>
              {#each context().acceptedFiles as file, index (file.name)}
                <FileUpload.Item {file}>
                  <div
                    class="grid grid-cols-12 gap-3 px-4 py-3 {index !== context().acceptedFiles.length - 1
