...
>
  <AngleSlider.Control class="absolute inset-0">
    <svg
      ...
      style="--size: {width}px; --thickness: {thickness}px; --percent: calc((var(--value) / 360) * 100);"
    >
      <title>Slider Ring</title>
      <!-- Track circle -->
      <circle
        class="stroke-gray-300 dark:stroke-gray-600 fill-transparent"
        style="--radius: calc(var(--size) / 2 - var(--thickness) / 2); cx: calc(var(--size) / 2); cy: calc(var(--size) / 2); r: var(--radius); stroke-width: var(--thickness);"
        cx="calc(var(--size) / 2)"
        cy="calc(var(--size) / 2)"
        r="var(--radius)"
    ...
    <AngleSlider.Thumb class="absolute top-0 right-0 bottom-0 left-[calc(50%-1.5px)] pointer-events-none h-full w-[3px] flex items-start outline-none">
      <span class="bg-gradient-to-br from-blue-500 to-purple-600 dark:from-blue-400 dark:to-purple-500 w-5 h-5 rounded-full flex-shrink-0 scale-125 shadow-lg shadow-blue-500/25 dark:shadow-blue-400/25 border-2 border-white dark:border-gray-800" />
    </AngleSlider.Thumb>
  </AngleSlider.Control>
